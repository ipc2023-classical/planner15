...
void GraphCreator::initialize(OperatorCost cost_type) {
    GraphCreator::cost_type = cost_type;
#ifdef USE_BLISS
    if (initialized){
        return;
    }
...
#ifdef USE_BLISS
unique_ptr<bliss::Digraph> GraphCreator::create_bliss_directed_graph() const {
    ...
    unique_ptr<bliss::Digraph> g(new bliss::Digraph());
    int idx = 0;
...
void GraphCreator::add_operator_directed_graph(unique_ptr<bliss::Digraph> &g, const Operator &op) const {
    int OP_COLOR = MAX_VALUE + get_adjusted_action_cost(op, cost_type);
...
Permutation GraphCreator::create_permutation_from_state_to_state(const GlobalState &from_state,
...
GraphCreator::create_permutation_from_state_to_state(const GlobalState &state,
...
static PluginTypePlugin<GraphCreator> _type_plugin(
    "GraphCreator",
    "TODO",
    "graph_creator");

static shared_ptr<GraphCreator> _parse(OptionParser &parser) {
    ...
    if (!parser.dry_run()) {
        ...
        return make_shared<GraphCreator>(opts);
    } else {
        return 0;
    }
}
